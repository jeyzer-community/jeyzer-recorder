...
# JMX connection user and password (optional)
if [ -z "$JEYZER_TARGET_JMX_USER" ]; then
  JEYZER_TARGET_JMX_USER=glassfish
  export JEYZER_TARGET_JMX_USER
fi

if [ -z "$JEYZER_TARGET_JMX_PASSWORD" ]; then  
  JEYZER_TARGET_JMX_PASSWORD=glassfish12
  export JEYZER_TARGET_JMX_PASSWORD
fi
...
# Dump access method (JMX or AdvancedJMX) 
JEYZER_RECORD_DUMP_METHOD=AdvancedJMX
...
LOG_FILE_SUFFIX=`echo $JEYZER_TARGET_JMX_ADDRESS | tr ':' '-'`
JEYZER_RECORDER_LOG_FILE="$JEYZER_RECORDER_HOME/log/jeyzer-recorder-jmx-"$JEYZER_RECORD_PROFILE"-"$LOG_FILE_SUFFIX".log"
